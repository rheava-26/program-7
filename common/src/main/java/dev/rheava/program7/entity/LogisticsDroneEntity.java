...
/**
 * The Logistics Drone — an unarmed flying courier that ferries the
 * Director's payment from the probe core out to an assembler mid-build.
 * It has no fight in it and doesn't try: shoot it down mid-run and its
 * cargo spills out as salvage, the resources gone before the assembler
 * ever saw them.
 */
public class LogisticsDroneEntity extends ProgramDroneEntity implements CourierUnit {
	@Nullable
	private BlockPos destination = null;
	private String job = "";
	private final Map<String, Integer> cargo = new HashMap<>();

	public LogisticsDroneEntity(EntityType<? extends PathAwareEntity> entityType, World world) {
		super(entityType, world);
		this.moveControl = new FlightMoveControl(this, 20, true);
		this.experiencePoints = 4;
	}

	public static DefaultAttributeContainer.Builder createLogisticsDroneAttributes() {

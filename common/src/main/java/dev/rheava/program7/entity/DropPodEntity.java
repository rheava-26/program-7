...
public class DropPodEntity extends Entity {
	...
	public void tick() {
		...
		if (this.age % 15 == 0) {
			this.getWorld().playSound(null, this.getBlockPos(), P7Sounds.DROP_POD_DESCENT.get(),
	...
	private void impact(ServerWorld world) {
		...
		world.playSound(null, pos, P7Sounds.DROP_POD_IMPACT.get(), SoundCategory.HOSTILE, 16.0f, 0.9f);
	...
	public static void spawnLandingComplement(ServerWorld world, BlockPos pos) {
		for (int i = 0; i < 2; i++) {
			SurveyorDroneEntity surveyor = P7Entities.SURVEYOR_DRONE.get().create(world);
			if (surveyor != null) {
		...
		for (int i = 0; i < 2; i++) {
			HarvesterDroneEntity harvester = P7Entities.HARVESTER_DRONE.get().create(world);
			if (harvester != null) {

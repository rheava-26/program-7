...
public class ScanPlayerGoal extends Goal {
	...
	public void start() {
		this.scanTicks = 0;
		this.drone.playSound(P7Sounds.DRONE_ALERT, 1.0f, 1.0f);
	}
	...
	public void tick() {
		...
		if (distanceSq <= SCAN_PROGRESS_RANGE * SCAN_PROGRESS_RANGE && this.drone.canSee(this.target)) {
			...
			if (this.scanTicks % beepInterval == 0) {
				...
				this.drone.playSound(P7Sounds.DRONE_SCAN_BEEP, 0.8f, pitch);
			}
	...
	private void completeScan() {
		if (this.target instanceof ServerPlayerEntity player) {
			...
			player.getServerWorld().playSound(null, player.getBlockPos(),
					P7Sounds.DRONE_INTERFERENCE, SoundCategory.HOSTILE, 1.0f, 1.0f);

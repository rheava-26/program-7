...
public final class InterferenceManager {
	...
	public static void register() {
		PayloadTypeRegistry.playS2C().register(InterferencePayload.ID, InterferencePayload.CODEC);
		ServerTickEvents.END_WORLD_TICK.register(InterferenceManager::tickWorld);
	...
	private static void tickWorld(ServerWorld world) {
		...
		for (ServerPlayerEntity player : world.getPlayers()) {
			...
			List<MobEntity> drones = world.getEntitiesByClass(MobEntity.class, searchBox,
					entity -> entity.getType() == P7Entities.SURVEYOR_DRONE
							|| entity.getType() == P7Entities.ATTACK_DRONE);
			...
			if (Math.abs(intensity - last) > 0.01f || keepAlive) {
				ServerPlayNetworking.send(player, new InterferencePayload(intensity, threatYaw));
